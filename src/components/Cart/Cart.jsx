    ...
    <div className="mt-10 md:mt-12 lg:mt-14 rounded-2xl border-2 border-[#E4E6EB] bg-white p-4 md:p-5 lg:p-6">
      ...
      <div className="mt-4 md:mt-5 lg:mt-6 space-y-3 md:space-y-4 lg:space-y-4">
        {selectedItems.map((item) =>
          <div key={item.id} className="rounded-2xl bg-[#F7F8FC] border-2 hover:border-violet-500 transition-colors duration-500 border-[#E4E6EB] p-3 md:p-4 lg:p-5 flex items-center justify-between gap-3 md:gap-4">
            ...
            <button onClick={()=>manageDelete(item)}
              type="button"
              className="text-xs md:text-sm lg:text-base font-semibold text-[#FF4F87] hover:text-violet-500 transition-colors duration-500"
            >
              Remove
            </button>
          </div>
